<template>
          ...
          <div class="space-y-6 sm:space-y-5" v-else>
            <span v-for="(column, index) in columns" :key="column.id">
              ...
              <div
                ...
                v-if="column.formtypes.slug == 'input-text' && column.is_form_hidden == 0"
              >
                <label
                  :for="column.column_name"
                  :class="column.is_form_hidden == 1 ? 'hidden' : ''"
                ...
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <div
                    :class="
                      (column.is_form_hidden == 1 ? 'hidden' : '',
                      'flex max-w-lg rounded-md shadow-sm')
                    "
                  >
                    <!-- FILE SIZE -->
                    <input
                      v-model="file_size"
                      v-if="column.column_name == 'file_size'"
                      disabled
                      type="text"
                      ...
                      :class="
                        errors['formdata.' + column.column_name]
                      ...
                      "
                      :aria-invalid="errors[column.column_name] ? 'true' : 'false'"
                      :aria-describedby="
                        errors[column.column_name] ? column.column_name + '-error' : ''
                      "
                    />
                    ...
                    <input
                      v-model="file_type"
                      v-else-if="column.column_name == 'file_type'"
                      disabled
                      type="text"
                      :name="column.column_name"
                      :disabled="column.is_disabled == 1"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="
                  column.formtypes.slug == 'input-text-with-na' &&
                  column.is_form_hidden == 0
                "
              >
                <label
                  :for="column.column_name"
                ...
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <div class="flex max-w-lg rounded-md shadow-sm">
                    <input
                      v-model="form[column.column_name]"
                      :type="column.is_form_hidden == 1 ? 'hidden' : 'text'"
                      :name="column.column_name"
                      :disabled="column.is_disabled == 1"
                      :id="column.column_name"
                      :autocomplete="column.column_name"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'password'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'email'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'input-number'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'input-date'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'input-date-month'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'input-time'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'input-datetime'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'dropdown'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
                ...
                <span
                  v-else-if="
                    column.is_foreignkey == 1 ||
                    column.reference_table !== null ||
                    column.reference_table !== ''
                  "
                >
                  <Combobox
                    :by="column.reference_table_column"
                    nullable
                    as="div"
                    v-model="selectedData[column['reference_table']]"
                    class="sm:col-span-2 sm:mt-0"
                  >
                    <div class="relative mt-2 flex-auto w-96">
                      <ComboboxInput
                        @keyup.enter="getDataFromForeignKey()"
                        @focus="
                          onFocuschange(
                            column.reference_table,
                            column.reference_table_column
                          )
                        "
                        :disabled="column.is_disabled == 1"
                        :class="[
 ...
 -->
                        ...
                        <ComboboxOption
                          @click="getDataFromForeignKey()"
                          v-for="cbdata in filterComboBoxSearch(
                            query[column.reference_table],
                            column.reference_table,
                            column.reference_table_column
                          )"
                          :key="cbdata.id"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'switch'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
                ...
                <Switch
                  v-model="form[column.column_name]"
                  :change="switchCtoogle(column, form[column.column_name])"
                  :disabled="column.is_disabled == 1"
                  :class="[
                    form[column.column_name] == 1 ? 'bg-indigo-600' : 'bg-gray-200',
                    column.is_disabled == 1
                ...
                >
                  ...
                  <span
                    aria-hidden="true"
                    :class="[
                      form[column.column_name] == 1 ? 'translate-x-5' : 'translate-x-0',
                ...
                <p
                  v-if="errors['formdata.' + column.column_name]"
                  class="mt-5 text-sm text-red-600"
                  :id="column.column_name + '-error'"
                >
                  {{
                    errors["formdata." + column.column_name][0].replace("formdata.", "")
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'file-photo'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
                ...
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <span
                    v-if="column.is_avatar_photo == 1"
                    class="inline-block h-80 w-80 overflow-hidden rounded-full bg-gray-100"
                  >
                  ...
                  <div class="flex max-w-lg rounded-md shadow-sm">
                    <input
                      @change="onFileChange($event, column.column_name)"
                      type="file"
                      :name="column.column_name"
                      :disabled="column.is_disabled == 1"
                      :id="column.column_name"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'file-document'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
                ...
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <div class="flex max-w-lg rounded-md shadow-sm">
                    <input
                      @change="onFileDocumentChange($event, column.column_name)"
                      type="file"
                      :name="column.column_name"
                      :disabled="column.is_disabled == 1"
                      :id="column.column_name"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'file-video'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
                >
                  {{ column.column_label }}
                  <span v-if="column.is_required == 1" class="text-red-600">*</span>
                </label>
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <span v-if="column.is_avatar_video == 1">
                    <video
                      class="inline-block h-80 w-80 overflow-hidden rounded-full bg-gray-100"
                      controls
                    >
                      <source :src="videodata[column.column_name]" />
                    </video>
                  </span>
                  <span v-else>
                  ...
                  <div class="flex max-w-lg rounded-md shadow-sm">
                    <input
                      @change="onVideoDocumentChange($event, column.column_name)"
                      type="file"
                      :name="column.column_name"
                      :disabled="column.is_disabled == 1"
                      :id="column.column_name"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'color-picker'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'signature'"
              >
                <label
                  :for="column.column_name"
                  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"
                ...
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <div>
                    <label class="text-base font-semibold text-gray-900"
                      >E-Signature Method</label
                    >
                    <p class="text-sm text-gray-500">
                      How do you prefer to saving e-signature?
                    </p>
                    <fieldset class="mt-4">
                      ...
                      >
                        <div
                          v-for="esignaturemethod in esignaturemethods"
                          :key="esignaturemethod.id"
                          class="flex items-center"
                        >
                          <input
                            v-model="selectedsignaturemethod"
                            :value="esignaturemethod.id"
                            :id="esignaturemethod.id"
                            name="esignature-method"
                            type="radio"
                            :checked="esignaturemethod.id === 'file-browse'"
                            class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-600"
                          />
                          <label
                            :for="esignaturemethod.id"
                            class="ml-3 block text-sm font-medium leading-6 text-gray-900"
                            >{{ esignaturemethod.title }}</label
                          >
                        </div>
                      </div>
                    </fieldset>
                    <div
                      ...
                      v-if="selectedsignaturemethod == 'signaturepad'"
                    >
                      <button
                        @click="openSignaturePad(column.column_name)"
                        type="button"
              ...
              <div
                class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                v-if="column.formtypes.slug == 'price'"
              >
                <label
                  :for="column.column_name"
                ...
                <div class="relative mt-1 rounded-md shadow-sm">
                  ...
                  <input
                    @keyup="formatPHP(column.column_name)"
                    ...
                    :class="
                      errors['formdata.' + column.column_name]
                        ? 'block w-full rounded-md border-red-300 pl-7 pr-12 text-red-900 placeholder-red-300 focus:border-red-500 focus:outline-none focus:ring-red-500 sm:text-sm'
                        : column.is_disabled == 1
                        ? 'block w-full rounded-md border-gray-300 pl-7 pr-12 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500 disabled:ring-gray-200'
                        : 'block w-full rounded-md border-gray-300 pl-7 pr-12 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm'
                    "
                    placeholder="0.00"
                    :aria-invalid="
                      errors['formdata.' + column.column_name] ? 'true' : 'false'
                    "
                    :aria-describedby="
                      errors['formdata.' + column.column_name]
                        ? column.column_name + '-error'
                        : form[column.column_name] + '-currency'
                    "
                  />
                  <div
              ...
              <div v-if="column.use_texteditor == 1">
                <div
                  class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5"
                  v-if="column.formtypes.slug == 'textarea'"
                >
                  <label
                    :for="column.column_name"
                    class="block text-sm font-medium text-gray-700"
                  ...
                  <div class="mt-1 sm:col-span-2 sm:mt-0">
                      ...
                      <span v-if="column.text_editor_id !== null">
                        <span v-if="column.texteditors.is_default == 1">
                          <span v-if="column.texteditors.slug == 'quill-editor'">
                            <QuillEditor
                              v-model:content="form[column.column_name]"
                              content-type="html"
                            ></QuillEditor>
                          </span>
                          <span v-else>
                            <textarea
                              ...
                              :class="
                                errors['formdata.' + column.column_name]
                                  ? 'block w-full rounded-md border-red-300 pr-10 text-red-900 placeholder-red-300 focus:border-red-500 focus:outline-none focus:ring-red-500 sm:text-sm'
                                  : column.is_disabled == 1
                                  ? 'block w-full rounded-md border-gray-300 pl-7 pr-12 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500 disabled:ring-gray-200'
                                  : 'block w-full rounded-md border-gray-300 pl-7 pr-12 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm'
                              "
                              :aria-invalid="
                                errors[column.column_name] ? 'true' : 'false'
                              "
                              :aria-describedby="
                    ...
                    >
                      {{
                        errors["formdata." + column.column_name][0].replace(
                          "formdata.",
                          ""
                        )
                      }}
            ...
            <!-- Multi Select -->
            <span v-if="title === 'WorkShift' || title === 'ShiftSetup'" class="">
                <div v-for="item in multiMenu" :key="item"   class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-2 pb-2">
                    <label :for="item" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">
                        {{ item }}
                                        ...
                                        <div class="sticky top-0 bg-white z-20 px-2 py-2 border-b">
                                            ...
                                            <span>
                                                <button
                                                    ...
                                                    @click.prevent="clearSearch(item)"
      ...
      <div class="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
        <button
          @click.prevent="editmode == true ? updateform() : submitform()"
          type="button"
  ...
  methods: {
    ...
    switchCtoogle(data, val) {
        if (data.column_name === 'with_start_date') {
            ...
            console.log('WAAAAAAAAAAAAA', data);

            // Determine the new disabled state based on the switch state
            const newDisabledState = val ? 0 : 1;

            // Update "second_start_date"
            const secondStartDateColumn = this.columns.find(col => col.column_name === 'second_start_date');
            if (secondStartDateColumn) {
                secondStartDateColumn.is_disabled = newDisabledState;
            }

            // Update "first_start_date"
            const firstStartDateColumn = this.columns.find(col => col.column_name === 'first_start_date');
            if (firstStartDateColumn) {
            firstStartDateColumn.is_disabled = newDisabledState;
            }
            if (newDisabledState==1){
                this.form[secondStartDateColumn.column_name] = null;
                this.form[firstStartDateColumn.column_name] = null;
            }
        }
    }
...
,
      ...
      this.columns.forEach((val) => {
        if (val.is_foreignkey == 1) {
          ...
          if (val.is_fk_getreferenced == 1) {
            this.form[val.column_name] = this.datavaluesreferencedother[
              val.reference_table_column
            ];
          ...
          if (val.where_column) {
            ...
            if (val.formtypes.slug == "dropdown") {
              axios
                .get("/getDataFromForeignKey", {
                  params: {
                    modelname: val.reference_table,
                    wherecolumn: val.where_column,
                    formvalue: val.where_column
                      ? val.where_column.split(".").length > 1
                        ? this.form[val.where_column.split(".")[1]].id
                        : this.form[val.where_column.split(".")[0]].id
                      : "",
            ...
            axios
              .get(route("getDataFromReferenceTable"), {
                params: {
                  modelname: val.reference_table,
          ...
          axios
            .get(route("getDataFromReferenceTableWhereColumn"), {
              params: {
                modelname: val.reference_table,
                wherecolumn: val.where_column,
            ...
            .then((response) => {
              ...
              if (val.display_data_column) {
                if (this.form[val.column_name]) {
                  this.form[val.display_data_column] = this.form[val.column_name][
                    val.display_data_column
                  ];
                }
              }
            });
          ...
          if (this.editmode == false) {
            axios
              .get(route("autogeneratecodes.getAutoGenCodeBySystemTableDetails"), {
                params: {
                ...
                if (matches) {
                  ...
                  if (stringsInsideBraces.length > 0) {
                    ...
                    stringsInsideBraces.forEach((val1, index) => {
                      concatenatedString += this.form[val1.split(".")[0]][
                        val1.split(".")[1]
                      ];
                    });
          ...
          if (
            val.column_name == "reference_table_column" &&
            this.datavalues["reference_table"] !== ""
          ) {
            axios
              .get(route("getTableColumnsFinal"), {
                params: {
    ...
    onVideoDocumentChange(e, columnname){
        var files = e.target.files || e.dataTransfer.files;
        if(!files.length) return;

        this.uploadVideo(files[0], columnname);
    },

    uploadVideo(file, columnname){
        console.log("File being uploaded:", file);
        console.log("Current video data:", this.videodata);
        var reader = new FileReader();
        var vm = this;

        reader.onload = (e) => {
            vm.video = e.target.result;
            this.videodata[columnname] = e.target.result;
      ...
      if (this.editmode == true) {
        ...
        this.columns.forEach((val, key) => {
          ...
          if (val.column_type == "boolean" || val.column_type == "smallint") {
            ...
            this.selectedData[val.reference_table] = this.datavalues[
              pluralizereferencecolumn
            ];
    ...
    async isautogenerate(id) {
      ...
      const response = await axios.get(
        route("autogeneratecodes.getAutoGenCodeBySystemTableDetails"),
        {
          params: {
    ...
    formatPHP(columnname) {
      this.form[columnname] = this.form[columnname]
        .toString()
        .replace(/\D/g, "")
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    },
    ...
    updateform() {
      ...
      axios
        .patch(route(this.routename + ".update", this.formid), { formdata: this.form })
        .then((response) => {
          console.log(response.data);
          if (response.data == "success") {
            if (this.ismainsidebar == false) {
  ...
  computed: {
    ...
    filterComboBoxSearch() {
      return (searchQuery, referencetable, referencetablecolumn, columnname) => {
        ...
        return this.filteredData[referencetable].filter((item) =>
          item[referencetablecolumn].toLowerCase().includes(query)
        );
      };
    },
  },
