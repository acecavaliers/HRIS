...
class EmployeeDependentController extends Controller
{
    ...
    {
        $validated = CommonController::getRequiredFields('EmployeeDependent', $request);

        $systemtable = SystemTable::where('table_name', 'employee_dependents')->first();
        ...
        $query = EmployeeDependent::create($arr);
        return 'success';
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $query = EmployeeDependent::where('id', $id)->update($arr);
        return 'success';
    }
    ...
    {
        EmployeeDependent::where('id', $id)->update(['deleted_by'=> Auth::user()->name]);
        EmployeeDependent::where('id', $id)->delete();
        return 'success';
    }

    public function getlist(Request $request){

        $query = EmployeeDependent::with('relationships', 'employees')->where('employee_id', $request->employee_id)->paginate($request->showrecords);
        return $query;
    }
