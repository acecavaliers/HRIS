...
class TrainingCategoryController extends Controller
{
    ...
    {
        return Inertia::render('Maintenance/TrainingCategory');
    }
    ...
    {
        $validated = CommonController::getRequiredFields('TrainingCategory', $request);

        $systemtable = SystemTable::where('table_name', 'training_categories')->first();
        ...
        if ($request->hasFile('file_video')) {
            ...
            $path = $file->storeAs('public/videos');

            $arr['file_video'] = $path;
            $arr['file_video'] = $originalname;
        }

        $arr['created_by'] = Auth::user()->name;
        $arr['training_id'] = 1;
        $query = TrainingCategory::create($arr);
        return 'success';
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $query = TrainingCategory::where('id', $id)->update($arr);
        return 'success';
    }
    ...
    {
        TrainingCategory::where('id',$id)->update(['deleted_by' => Auth::user()->name]);
    ...
    {
        $query = TrainingCategory::with('trainings')->paginate($request->showrecords);
        return $query;
    }
