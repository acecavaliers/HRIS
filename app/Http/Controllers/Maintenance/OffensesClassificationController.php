...
class OffensesClassificationController extends Controller
{
    /**
    ...
    {
        return Inertia::render('Maintenance/OffenseClassification');
    }
    ...
    {
        $validated = CommonController::getRequiredFields('OffensesClassification', $request);

        $systemtable = SystemTable::where('table_name', 'offense_classifications')->first();
        ...
        $query = OffensesClassification::create($arr);
        return 'success';
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $query = OffensesClassification::where('id', $id)->update($arr);
        return 'success';
    }
    ...
    {
        OffensesClassification::where('id', $id)->update(['deleted_by'=> Auth::user()->name]);
        OffensesClassification::where('id', $id)->delete();
        return 'success';
    }

    public function getlist(Request $request){

        $query = OffensesClassification::with('offensedetails', 'codeofdisciplines.offensecategories')->paginate($request->showrecords);
        return $query;
    }
    ...
    public function searchData(Request $request){
        ...
        /*     return $request->is_active_all; */
            ...
            $val = 0;
            $query = OffensesClassification::query();
