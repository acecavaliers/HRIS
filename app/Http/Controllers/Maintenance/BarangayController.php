...
class BarangayController extends Controller
{
    ...
    {
        return Inertia::render('Maintenance/Barangay');
    }
    ...
    {
        $validated = CommonController::getRequiredFields('Barangay', $request);

        $systemtable = SystemTable::where('table_name', 'barangays')->first();
    ...
    public function update(Request $request, string $id)
    {
        ...
        $query = Barangay::where('id', $id)->update($arr);
        return 'success';
    }
    ...
    {
        Barangay::where('id', $id)->update(['deleted_by'=> Auth::user()->name]);
    ...
    {
        $query = Barangay::with('cities')->paginate($request->showrecords);
        return $query;
    }

    public function getactivelist(Request $request){

        $query =  Barangay::with('cities')->where(array('city_id' => $request->cityID, 'is_active' => 1))->get();
        return $query;
